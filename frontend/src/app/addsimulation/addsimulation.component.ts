...
export class AddsimulationComponent implements OnInit, AfterViewInit {
    ...
    addInputControl() {
        ...
        const lastTier = this.seuils_ep.value[this.seuils_ep.length - 1];
        const newThreshold = (lastTier ? lastTier.seuil : 0) + 1;
        const lastPrix = lastTier ? lastTier.prix : 0;

        const trancheForm = this.fb.group({
            seuil: new FormControl(newThreshold, {validators: [Validators.required]}),
            prix: new FormControl(lastPrix + 0.0001, {validators: [Validators.required, Validators.minLength(4)]}),
        });
    ...
    addInputControl_a() {
        ...
        // Calculate the new threshold value
        const lastTier = this.seuils_a.value[this.seuils_a.length - 1];
        const newThreshold = (lastTier ? lastTier.seuil : 0) + 1;
        const lastPrix = lastTier ? lastTier.prix : 0;
